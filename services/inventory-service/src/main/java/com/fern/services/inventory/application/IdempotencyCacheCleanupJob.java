...
public class IdempotencyCacheCleanupJob extends BaseRepository {
  ...
  @SchedulerLock(name = "inventory-idempotency-cache-cleanup", lockAtMostFor = "PT15M", lockAtLeastFor = "PT30S")
  public void cleanupExpired() {
    try {
      ...
      if (deleted > 0) {
        log.info("inventory idempotency cache cleanup deleted {} expired rows", deleted);
      } else {
        log.debug("inventory idempotency cache cleanup: no expired rows");
      }
    } catch (RuntimeException e) {
      log.warn("inventory idempotency cache cleanup failed: {}", e.getMessage());
    }
  }
