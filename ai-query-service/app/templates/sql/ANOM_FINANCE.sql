SELECT
    ...
    avgIf(operating_profit, business_date < toDate('{{ from_date }}')) AS baseline_daily_avg,
    stddevSampIf(operating_profit, business_date < toDate('{{ from_date }}')) AS baseline_daily_stddev,
    countIf(business_date < toDate('{{ from_date }}')) AS baseline_days,
    if(current_days = 0, 0, current_operating_profit / current_days) AS current_daily_avg,
