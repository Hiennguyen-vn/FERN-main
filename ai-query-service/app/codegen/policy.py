...
from app.rbac.policy import TEMPLATE_ROLE_RESTRICTIONS
from app.query_policy import finance_sensitive_tables

# Tables that appear only in finance-restricted templates (must align with T24–T27).
FINANCE_SENSITIVE_TABLES: frozenset[str] = finance_sensitive_tables()
...
def check_codegen_finance_access(tables_lower: frozenset[str], roles: frozenset[str] | set[str]) -> tuple[bool, str | None]:
    hit = tables_lower & FINANCE_SENSITIVE_TABLES
    if not hit:
        return True, None
