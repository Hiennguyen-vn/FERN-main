...
function PublicShell({
  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(180deg,#fffaf1_0%,#fff7ed_22%,#f8fafc_100%)] text-slate-900">
...
function PublicStatePanel({
  ...
  return (
    <div className="mx-auto flex max-w-2xl flex-col items-center rounded-[28px] border border-slate-200 bg-white/85 px-6 py-10 text-center shadow-[0_30px_120px_rgba(15,23,42,0.08)] backdrop-blur">
      <div className="mb-5 flex h-14 w-14 items-center justify-center rounded-full border border-slate-200 bg-slate-50 text-slate-700">
        {icon}
      </div>
...
function ReceiptPanel({
  ...
  return (
    <section className="mx-auto grid max-w-5xl gap-6 lg:grid-cols-[minmax(0,1.15fr)_360px]">
      <div className="rounded-[28px] border border-slate-200 bg-white/90 p-6 shadow-[0_32px_120px_rgba(15,23,42,0.08)] backdrop-blur">
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.3em] text-slate-500">Request received</p>
            <h2 className="mt-2 text-3xl font-semibold tracking-tight text-slate-950">
              {receipt.tableName || receipt.tableCode || 'Table order'}
            </h2>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
              Staff will confirm and advance the order from the in-store POS. Public payment is not available in this flow.
            </p>
          </div>
        ...
        <div className="mt-6 grid gap-4 rounded-3xl border border-slate-200 bg-slate-50/80 p-4 sm:grid-cols-2">
        ...
        <div className="mt-6">
          ...
          <div className="mt-3 space-y-3">
            {(receipt.items || []).map((item) => (
              <div key={`${item.productId || item.productCode}-${item.note || ''}`} className="flex items-start justify-between gap-4 rounded-2xl border border-slate-200 bg-white px-4 py-3">
                <div>
      ...
      <aside className="space-y-4 lg:sticky lg:top-6 lg:self-start">
        <div className="rounded-[28px] border border-slate-200 bg-white/90 p-5 shadow-[0_24px_90px_rgba(15,23,42,0.08)] backdrop-blur">
          <p className="text-[11px] font-semibold uppercase tracking-[0.28em] text-slate-500">Status refresh</p>
          <p className="mt-3 text-sm leading-6 text-slate-600">
            This receipt refreshes every 15 seconds while the page is visible. Use manual refresh if staff just updated the order.
          </p>
          <div className="mt-5 flex flex-col gap-3">
            ...
            <Button className="h-11 justify-center gap-2" onClick={onContinueOrdering} disabled={!canContinueOrdering}>
              Continue ordering
              <ArrowRight className="h-4 w-4" />
            </Button>
          </div>
          {tableUnavailableMessage ? <p className="mt-3 text-xs leading-5 text-amber-700">{tableUnavailableMessage}</p> : null}
        </div>

        <div className="rounded-[24px] border border-slate-200 bg-slate-950 px-5 py-4 text-slate-100 shadow-[0_24px_90px_rgba(15,23,42,0.16)]">
          <p className="text-[11px] font-semibold uppercase tracking-[0.28em] text-slate-400">What happens next</p>
          <p className="mt-3 text-sm leading-6 text-slate-200">
            Your request is already in the staff queue. The cashier or service team will approve it, prepare the items, and manage payment at the table.
          </p>
        </div>
      </aside>
    </section>
...
export default function PublicOrderPage() {
  ...
  const header = (
    <header className="rounded-[32px] border border-amber-100 bg-white/85 px-5 py-5 shadow-[0_18px_80px_rgba(15,23,42,0.06)] backdrop-blur sm:px-6 lg:px-8">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div className="max-w-3xl">
          <p className="text-[11px] font-semibold uppercase tracking-[0.34em] text-amber-700">Public dining</p>
          <h1 className="mt-3 text-3xl font-semibold tracking-tight text-slate-950 sm:text-4xl">
            Order directly from your table
          </h1>
          <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">
            Send an order request to the in-store staff queue. Public payment is not available here yet, and order status updates after staff action.
          </p>
        </div>

        <div className="grid gap-3 rounded-[24px] border border-slate-200 bg-slate-50/85 px-4 py-4 text-sm text-slate-700 sm:grid-cols-2 sm:gap-4 lg:min-w-[360px] lg:grid-cols-1">
          ...
          <div className="flex items-start gap-3">
            <UtensilsCrossed className="mt-0.5 h-4 w-4 text-slate-500" />
            <div>
  ...
  const renderCartPanel = (variant: 'desktop' | 'mobile') => (
    <div className={cn(
      'rounded-[28px] border border-slate-200 bg-white/92 p-5 shadow-[0_24px_90px_rgba(15,23,42,0.08)] backdrop-blur',
      variant === 'desktop' && 'lg:sticky lg:top-6',
    )}>
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.28em] text-slate-500">Cart</p>
          <h2 className="mt-2 text-2xl font-semibold tracking-tight text-slate-950">Review this table request</h2>
        </div>
        <Badge className="rounded-full border border-amber-200 bg-amber-50 px-3 py-1 text-[11px] font-semibold text-amber-700">
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  invalid ? 'border-amber-300 bg-amber-50/70' : 'border-slate-200 bg-slate-50/70',
                )}
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                ...
                <div className="mt-3">
                  ...
                  />
                  {invalid ? <p className="mt-2 text-xs leading-5 text-amber-700">This line needs review before you can submit again.</p> : null}
                </div>
              </div>
            );
          })
  ...
  return (
    <PublicShell header={header}>
      {canResumeLastOrder ? (
        <div className="mb-5 flex flex-col gap-3 rounded-[24px] border border-slate-200 bg-white/85 px-5 py-4 shadow-[0_16px_60px_rgba(15,23,42,0.05)] sm:flex-row sm:items-center sm:justify-between">
          <div>
          ...
          <div className="flex gap-3">
            ...
            <Button className="h-10 gap-2" onClick={() => openReceipt(lastOrderToken)}>
              Resume order status
              <ArrowRight className="h-4 w-4" />
            </Button>
          </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
        <section className="rounded-[30px] border border-slate-200 bg-white/88 p-5 shadow-[0_28px_110px_rgba(15,23,42,0.07)] backdrop-blur sm:p-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.28em] text-slate-500">Menu</p>
              <h2 className="mt-2 text-3xl font-semibold tracking-tight text-slate-950">
            ...
            <div className="flex w-full max-w-md items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2.5">
          ...
          {menuQuery.isLoading ? (
            ...
            <div className="mt-8 rounded-[24px] border border-dashed border-slate-300 bg-slate-50 px-5 py-12 text-center">
                ...
                <div key={category.code} ref={(node) => {
                  ...
                  <div className="flex items-end justify-between gap-3">
                    <div>
                      ...
                      <h3 className="mt-2 text-2xl font-semibold tracking-tight text-slate-950">{category.label}</h3>
                    </div>
                  ...
                  <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                    {category.items.map((item) => {
                      ...
                      return (
                        <article key={String(item.productId || item.code)} className="group overflow-hidden rounded-[24px] border border-slate-200 bg-slate-50/70 transition hover:-translate-y-0.5 hover:border-slate-300 hover:bg-white">
                          <div className="aspect-[16/10] bg-[linear-gradient(135deg,#f8fafc_0%,#fef3c7_100%)]">
                            {item.imageUrl ? (
                              <img src={String(item.imageUrl)} alt={productName(item)} className="h-full w-full object-cover" />
                            ) : (
                              <div className="flex h-full items-center justify-between px-5">
                                <div>
                                  <p className="text-[11px] font-semibold uppercase tracking-[0.28em] text-slate-500">{formatPublicLabel(item.categoryCode, 'Menu')}</p>
                                  <p className="mt-2 text-2xl font-semibold tracking-tight text-slate-900">{String(item.code || '').slice(0, 8) || 'Menu'}</p>
                                </div>
                                <div className="flex h-16 w-16 items-center justify-center rounded-full border border-white/70 bg-white/60 text-lg font-semibold text-slate-700">
                          ...
                          <div className="p-4">
                            <div className="flex items-start justify-between gap-3">
                              <div>
                                <h4 className="text-lg font-semibold tracking-tight text-slate-950">{productName(item)}</h4>
                                <p className="mt-1 text-sm text-slate-500">{String(item.code || formatPublicLabel(item.categoryCode, 'Menu item'))}</p>
                              </div>
                              <p className="text-sm font-semibold text-slate-950">{formatCurrency(item.priceValue, currencyCode)}</p>
                            </div>
      ...
      {isMobile ? (
        <>
          <button
            type="button"
            className="fixed inset-x-4 bottom-4 z-30 flex items-center justify-between gap-4 rounded-full border border-slate-900 bg-slate-900 px-5 py-3 text-left text-white shadow-[0_18px_60px_rgba(15,23,42,0.22)]"
            onClick={() => setCartOpen(true)}
          >
            <span>
              <span className="block text-[11px] font-semibold uppercase tracking-[0.28em] text-slate-300">Cart</span>
          ...
          <Sheet open={cartOpen} onOpenChange={setCartOpen}>
            <SheetContent side="bottom" className="max-h-[92vh] overflow-y-auto rounded-t-[28px] border-slate-200 bg-[#fffaf3] px-4 pb-8 pt-5">
