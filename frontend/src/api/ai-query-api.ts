...
/** Last few turns sent to the AIA-gent backend for lightweight follow-ups (same session). */
...
/** Reviewer returns structured issues from the AIA-gent reviewer schema. */
...
export interface AiQueryResponse {
  ...
  /** JSON-safe row slice when preview_max_rows was sent (see AIA-gent compat adapter). */
...
export const aiQueryApi = {
  ...
  async ready(token?: string | null): Promise<AiServiceReadiness | null> {
    try {
      ...
      // 503 + JSON body from AIA-gent means degraded, not network failure
  ...
  /**
   * Submit a natural-language question to the AIA-gent service.
