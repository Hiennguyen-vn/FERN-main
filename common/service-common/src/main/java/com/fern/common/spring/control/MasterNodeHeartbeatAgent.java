package com.fern.common.spring.control;

import com.fern.common.spring.config.FernServiceProperties;
...
public class MasterNodeHeartbeatAgent {
  ...
  @CircuitBreaker(name = "master-node", fallbackMethod = "sendHeartbeatFallback")
  ...
  private HeartbeatResponse sendHeartbeatFallback(Throwable ex) {
    log.debug("master-node CB open or call failed for {}: {}", serviceName, ex.toString());
    return null;
  }
