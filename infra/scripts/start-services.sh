  ...
  if [[ "$name" == "gateway" ]]; then
    if $DEV_MODE; then
      nohup env \
        ...
        AI_QUERY_SERVICE_URL="${AI_QUERY_SERVICE_URL}" \
        GATEWAY_HTTPCLIENT_RESPONSE_TIMEOUT="${GATEWAY_HTTPCLIENT_RESPONSE_TIMEOUT:-180s}" \
        GATEWAY_AI_QUERY_TIMEOUT="${GATEWAY_AI_QUERY_TIMEOUT:-180s}" \
        INTERNAL_SERVICE_TOKEN="${resolved_internal_token}" \
        INTERNAL_SERVICE_ALLOWLIST="${INTERNAL_SERVICE_ALLOWLIST:-}" \
        JWT_SECRET="${JWT_SECRET:-}" \
        AUTH_COOKIE_NAME="${AUTH_COOKIE_NAME:-dorabets_session}" \
        ...
        GATEWAY_CORS_ALLOWED_HEADERS="${GATEWAY_CORS_ALLOWED_HEADERS:-Authorization,Content-Type,X-Correlation-ID}" \
        GATEWAY_CORS_EXPOSED_HEADERS="${GATEWAY_CORS_EXPOSED_HEADERS:-X-Correlation-ID,X-Gateway-Upstream-Service}" \
        GATEWAY_CIRCUIT_SLOW_CALL_THRESHOLD="${GATEWAY_CIRCUIT_SLOW_CALL_THRESHOLD:-}" \
        GATEWAY_CIRCUIT_OPEN_WAIT="${GATEWAY_CIRCUIT_OPEN_WAIT:-}" \
        DEBUG="${DEBUG:-false}" \
        LOGGING_LEVEL_ROOT="${LOGGING_LEVEL_ROOT:-INFO}" \
        LOGGING_LEVEL_ORG_SPRINGFRAMEWORK="${LOGGING_LEVEL_ORG_SPRINGFRAMEWORK:-INFO}" \
        LOGGING_LEVEL_ORG_SPRINGFRAMEWORK_JDBC="${LOGGING_LEVEL_ORG_SPRINGFRAMEWORK_JDBC:-INFO}" \
        OTEL_SAMPLE_RATIO="${OTEL_SAMPLE_RATIO:-0.0}" \
        JAVA_OPTS="${JAVA_OPTS:-}" \
    ...
    if $DEV_MODE; then
      nohup env \
        ...
        S3_BUCKET_FINANCE_DOCUMENTS="${S3_BUCKET_FINANCE_DOCUMENTS:-}" \
